...
# @description This script will install (Mod) Ansible Autodoc. The script heavily borrows from
...
#
#   ```
#   curl -sSL https://raw.githubusercontent.com/megabyte-labs/mod-ansible-autodoc/master/local/install.sh | sh
#   ```

RELEASES_URL="https://github.com/megabyte-labs/mod-ansible-autodoc/releases"
...
if [ "${OS}" != "linux" ] && { [ "${ARCH}" = "ppc64" ] || [ "${ARCH}" = "ppc64le" ];}; then
  ...
  BINARY="mod-ansible-autodoc-${OS}-${ARCH}"
  if [ "$OS" = "windows" ]; then
    BINARY="$BINARY.exe"
  fi
fi
...
INSTALL_NAME="mod-ansible-autodoc"
if [ "$OS" = "windows" ]; then
